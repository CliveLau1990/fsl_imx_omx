...
int VpuSeqInit(DecHandle InVpuHandle, VpuDecObj* pObj ,VpuBufferNode* pInData,int* pOutRetCode,int * pNoErr) 
{
	...
	*pNoErr=1;	//set default: no error
	...
	else
	{
		...
		if(0!=headerLen)
		{
			(void)VpuFillData(InVpuHandle,pObj,pHeader,headerLen,1,0);
...
//#endif			
		...
		if(outInfo.indexFrameDecoded==VPU_OUT_DEC_INDEX_EOS)
		{
			...
			*pOutRetCode=(*pOutRetCode)|VPU_DEC_NO_ENOUGH_BUF;
			//workaround for gts com.google.android.exoplayer.gts.DashTest#testWidevineH264AdaptiveWithSeekingCenc
			//in gts package r5.0_r2
			if((pObj->nDecResolutionChangeEnabled == 0) && ((outInfo.decodingSuccess>>20) & 0x1 )){
				int used_num = VpuQueryVpuHoldBufNum(pObj);
				//all avaiable buffer has been queued into decoder, except 2 buffers are left in surface
				//can't get more output buffer for vpu decoder
				if(used_num + 3 > pObj->frameNum && used_num < pObj->frameNum){
					*pOutRetCode &= ~VPU_DEC_NO_ENOUGH_BUF;
					VPU_API("remove VPU_DEC_NO_ENOUGH_BUF flag");
				}
			}
		}
#endif
	...
	if(CPU_IS_MX6X())
	{
		...
		for(i=0;i<pVpuObj->obj.frameNum;i++)
		{
			...
			if(pVpuObj->obj.frameBufState[i]==VPU_FRAME_STATE_DEC)
			{
				...
				(void)vpu_DecClrDispFlag(pVpuObj->handle,i);
			}
#endif
		...
		if(CPU_IS_MX6X()){
			...
			*/
			if(VPU_V_VC1_AP==pObj->CodecFormat){
				...
				temp[0]=temp[1]=0;
				temp[2]=0x1;
				(void)VpuFillData(pVpuObj->handle,pObj,temp,3,1,0);
			}
			if(startFrameOK==0){
