...
VpuDecRetCode VPU_DecLoad()
{
  ...
  memset(&tPALCfg, 0, sizeof(sPALConfig));
  memset(&tTestVPUDriverCfg, 0, sizeof(VPU_DRIVER_CFG));
...
VpuDecRetCode VPU_DecOpen(VpuDecHandle *pOutHandle, VpuDecOpenParam * pInParam,VpuMemInfo* pInMemInfo)
{
  ...
  uParamVal[0] = (u_int32)pMemPhy->pPhyAddr;
  ...
  pObj->uPictureStartLocationPre = (u_int32)pObj->pBsBufPhyStart;
...
static void VpuUpdatePos(VpuDecObj* pObj, unsigned int len)
{
  ...
  {
    VPU_Stream_FeedBuffer(pObj->uStrIdx, (u_int32)(pObj->pBsBufPhyStart + pObj->nBsBufOffset
          ...
          if(!pObj->needDiscardEvent)
          {
            ...
            for (i=0;i<pObj->frameNum;i++){
              if (pObj->frameBufState[i] == VPU_FRAME_STATE_DEC){
                ...
                sVPUFsParams.ulFsLumaBase[0]   = (u_int32)pInFrameBuf->pbufY;
                sVPUFsParams.ulFsLumaBase[1]   = (u_int32)pInFrameBuf->pbufY + pInFrameBuf->nStrideY * pObj->picHeight / 2;
                sVPUFsParams.ulFsChromaBase[0] = (u_int32)pInFrameBuf->pbufCb;
                sVPUFsParams.ulFsChromaBase[1] = (u_int32)pInFrameBuf->pbufCb + pInFrameBuf->nStrideY * pObj->picHeight / 4;
...
VpuDecRetCode VPU_DecGetInitialInfo(VpuDecHandle InHandle, VpuDecInitInfo * pOutInitInfo)
{
  ...
  pOutInitInfo->nMinFrameBufferCount = psSeqInfo->uNumDPBFrms+1;
  pOutInitInfo->nBitDepth = psSeqInfo->uBitDepthLuma;
  pObj->nMinFrameBufferCount = psSeqInfo->uNumDPBFrms+1;
...
VpuDecRetCode VPU_DecOutFrameDisplayed(VpuDecHandle InHandle, VpuFrameBuffer* pInFrameBuf)
{
  ...
  {
    sVPUFsParams.ulFsId            = index;
    sVPUFsParams.ulFsLumaBase[0]   = (u_int32)pInFrameBuf->pbufMvCol;
