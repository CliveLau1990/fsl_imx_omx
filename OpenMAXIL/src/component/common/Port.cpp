...
Port::Port(ComponentBase *pBase, OMX_U32 nPortIdx)
{
    ...
    fsl_osal_memset(sPortFormat,0,sizeof(OMX_VIDEO_PARAM_PORTFORMATTYPE) * MAX_PORT_FORMAT_NUM);
    fsl_osal_memset(BufferHdrArry,0,sizeof(OMX_BUFFERHEADERTYPE *) * MAX_PORT_BUFFER);
    fsl_osal_memset(MarkDatas,0,sizeof(OMX_MARKTYPE) * MAX_PORT_BUFFER);
        ...
        OMX_U32 nSizeBytes)
{
    ...
    LOG_DEBUG("Port %d AllocateBuffer size %d nBufferSize %d.\n",nPortIndex, nSizeBytes, sPortDef.nBufferSize);
...
OMX_ERRORTYPE Port::Disable()
{
    ...
    if(bTunneled == OMX_FALSE) {
        ...
        // bugfix, set bEnabled=OMX_FALSE otherwise port is still enabled even after Disable().
        sPortDef.bEnabled = OMX_FALSE;
    }
