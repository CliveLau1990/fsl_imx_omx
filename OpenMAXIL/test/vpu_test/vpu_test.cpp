...
OMX_U32 get_h264_frame_length(HTEST *hTest)
{
    ...
    if(bFound) {
        ...
        printf("get_h264_frame_length len=%d \n",nIndex-3);
        return nIndex-3;
    }

    return 0;   // EOF reached
}
OMX_U32 get_hevc_frame_length(HTEST *hTest)
{
    OMX_S32 nIndex = 0, nChar;
    OMX_U32 nInputBuf = 0xffffffff;
    OMX_U32 nStartCode = 0x00000001;
    OMX_BOOL bFound = OMX_FALSE;
    OMX_BOOL bSlice = OMX_FALSE;
    FILE *fp = hTest->pInFile;
    ...
    for (nIndex = 0; (nChar = fgetc(fp)) != EOF; nIndex++)  {
        nInputBuf = (nInputBuf << 8) | (nChar & 0xff);
        if (NALU_START_CODE == (nInputBuf & 0xffffffff)) {
            nChar = fgetc(fp);
            if((nChar&0x1f) < 6 && (nChar&0x1f) > 0) {
                if(bSlice == OMX_TRUE)
                    bFound = OMX_TRUE;
    ...
    printf("get_hevc_frame_length index=%d,found=%d\n",nIndex,bFound);
    if(bFound) {
        fseek(fp, -(nIndex+1), SEEK_CUR);
        return nIndex-3;
    }
...
OMX_ERRORTYPE port_flush(HTEST *hTest, OMX_U32 nPortIndex)
{
    ...
    OMX_ERRORTYPE ret = OMX_ErrorNone;
    hComponent = hTest->hComponent;
    ret = SendCommand(hTest, OMX_CommandFlush, nPortIndex, NULL,OMX_TRUE);
    if(ret != OMX_ErrorNone)
        return ret;
    printf("port flush done.\n");
    return OMX_ErrorNone;
}
        ...
        FORMAT fmt)
{
    ...
    if(fmt == HEVC) {
        hTest->role = (OMX_STRING)"video_decoder.hevc";
        hTest->get_video_frame_len = get_hevc_frame_length;
        hTest->totalLen = 0;
    }
    return hTest;
}
...
int main(int argc, char *argv[])
{
    ...
    if(fmt == HEVC)
        component = (OMX_STRING)"OMX.Freescale.std.video_decoder.hevc.hw-based";
